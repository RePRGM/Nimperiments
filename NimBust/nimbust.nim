...
from std/net import newContext, SslCVerifyMode, TimeoutError
from std/httpcore import HttpHeaderValues, contains
from std/uri import encodeUrl
from std/osproc import execCmd

type ThreadData = tuple[doAppend: bool, wordlist: seq[string], baseUrl: string, extension: seq[string], output: string, rDelay: float, timeout: int, userAgent: string, quietMode: bool]
...
when isMainModule:
  var p = newParser:
    ...
    flag("-r", "--recursion", help="Turn recursion on", hidden=true)
    ...
    option("--dt", help="Specify delay between thread creation in seconds", default=some("0"))
    option("--dr", help="Specify delay between each HTTP request in seconds", default=some("1"))
    option("--ua", help="Specify a user agent", default=some("NimBust/1.0.0"))
    option("--timeout", help="Specify HTTP request timeout in seconds", default=some("10"))
  
  var cmdLineOpt: tuple[doAppend: bool, threads: int, wordlist: string, url: string, output: string, extension: seq[string], tDelay: int, rDelay: float, timeout: int, userAgent: string, quietMode: bool]
...
proc bruteForce(options: tuple): void =
  ...
  proc saveToFile(value: string): void =
    try:
      ...
      stdout.styledWriteLine(fgRed, fmt "(Error)", fgDefault, fmt "\t(Could not save to output file {options.output})")  
    except: 
  ...
  proc doRequest(urlString: string, word: string): void =
    try:
      ...
      if not options.quietMode:
        ...
        elif response.status.contains(@["301", "307", "308"].HttpHeaderValues) or "302" in response.status: 
          stdout.styledWriteLine(fgYellow, fmt "({response.status.splitWhitespace()[0]})", fgDefault, fmt "\t/{word}")
        
        elif "404" notin response.status: 
      ...
      var error = getCurrentException()
      stdout.styledWriteLine(fgRed, fmt "(Error)", fgDefault, fmt "\t({error.msg})\t/{word}")
  ...
  if options.extension.len != 0:
    ...
    for word in options.wordlist:
      encodedWord = if options.doAppend: word.encodeUrl(false) & "/" else: word.encodeUrl(false)
    ...
    for word in options.wordlist:
      encodedWord = if options.doAppend: word.encodeUrl(false) & "/" else: word.encodeUrl(false)
      doRequest(fmt "{options.baseUrl}/{encodedWord}", fmt "{encodedWord}")
      sleep(int(options.rDelay*1000))
...
+=======================================================================================+"""
  if subWordLists.len < 5 or subWordLists[0].len < 1000:
    echo "(Notice)\t(Single thread mode enabled!)"
    for i in 0 ..< subWordLists.len:
      bruteForce((doAppend: options.doAppend, wordlist: subWordLists[i], baseUrl: options.url, extension: options.extension, output: options.output, rDelay: options.rDelay, timeout: options.timeout, userAgent: options.userAgent, quietMode: options.quietMode))
  else:
    for i in 0 ..< options.threads: 
      createThread(threads[i], bruteForce, (doAppend: options.doAppend, wordlist: subWordLists[i], baseUrl: options.url, extension: options.extension, output: options.output, rDelay: options.rDelay, timeout: options.timeout, userAgent: options.userAgent, quietMode: options.quietMode))
